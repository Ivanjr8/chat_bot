...
filtro_nome = st.text_input("Digite parte do nome do professor")

# 🔧 Função para garantir tipos nativos
...
if filtro_nome:
    resultados = db.buscar_professores(filtro_nome)

    if resultados:
        df = pd.DataFrame(resultados)
        st.dataframe(df)

        nomes = df["NO_NOME_PROFESSOR"].tolist()
        selecionado = st.selectbox("Selecione um professor", nomes)

        if selecionado:
            dados_prof = df[df["NO_NOME_PROFESSOR"] == selecionado].iloc[0]

            with st.form("form_edit"):
                novo_nome = st.text_input("Nome", value=dados_prof["NO_NOME_PROFESSOR"])
                nova_escola = st.text_input("Escola", value=dados_prof["NO_ESCOLA"])
                atualizar = st.form_submit_button("💾 Atualizar")
                excluir = st.form_submit_button("🗑️ Excluir")
                
                                    

                pk_professor = limpar_valor(dados_prof["PK_CO_PROFESSOR"])
                ...
                if atualizar:
                    ...
                    cursor.execute("""
                        ...
                        SET NO_NOME_PROFESSOR = ?
                        WHERE PK_CO_PROFESSOR = ?
                    """, (str(novo_nome), pk_professor))
                    db.conn.commit()
                ...
                if excluir:
                    ...
                    cursor = db.conn.cursor()
                    cursor.execute("""
                        DELETE FROM TB_013_PROFESSORES
                        WHERE PK_CO_PROFESSOR = ?
                    """, (pk_professor,))
                    db.conn.commit()
                    st.warning("🗑️ Professor excluído.")
                    time.sleep(2)  # Espera 5 segundos
        ...
        st.info("🔎 Nenhum professor encontrado com esse nome.")
st.header("➕ Cadastrar Novo Professor")

with st.form("form_novo_professor"):
    novo_nome = st.text_input("Nome do novo professor")
    nova_escola = st.text_input("Escola do novo professor")
    novo_disciplina = st.text_input("Disciplina nova professor")
    ...
    if cadastrar:
        ...
        # Conversão explícita para evitar erro de tipo
        professor = str(novo_nome)
        escola = int(nova_escola)
        disciplina = int(novo_disciplina)
       
        try:
            cursor.execute("""
                INSERT INTO TB_013_PROFESSORES (NO_NOME_PROFESSOR, FK_ID_ESCOLA, FK_CO_DISCIPLINA )
                VALUES (?, ?, ?)
            """, (professor, escola, disciplina))
            db.conn.commit()
